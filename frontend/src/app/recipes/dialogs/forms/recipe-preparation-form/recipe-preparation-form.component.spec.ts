...
describe('RecipePreparationFormComponent', () => {
  ...
  it('reorders a dragged step without losing its control or validation state', () => {
    ...
    expect(firstStep.invalid).toBeTrue();
  });
  ...
  it('prevents adding a 100th step and allows adding after removal', () => {
    ...
    expect(addButton.getAttribute('aria-describedby')).toBe('step-limit');
    expect(fixture.nativeElement.querySelector('#step-limit')?.textContent).toContain(
      'Maximal 99 Schritte möglich.'
    );
