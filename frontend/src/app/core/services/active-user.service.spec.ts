...
interface ActiveUserServiceTestContext {
  ...
  userBackendService: jasmine.SpyObj<UserBackendService>;
}
...
function createContext(
  getUserById: (userId: number) => Promise<User> = (): Promise<User> =>
    Promise.reject(new HttpErrorResponse({ status: 404 }))
): ActiveUserServiceTestContext {
  const snackBarService = jasmine.createSpyObj<SnackBarService>('SnackBarService', [
    'open',
  ]);
  ...
  userBackendService.getUserById.and.callFake(getUserById);
...
describe('ActiveUserService', () => {
  ...
  it('persists an explicitly selected user and reports the selection', () => {
    ...
    expect(localStorage.getItem(ACTIVE_USER_STORAGE_KEY)).toBe(
      JSON.stringify(USER)
    );
  ...
  it('restores and refreshes a persisted user without a selection notification', async () => {
    ...
    const { service, snackBarService, userBackendService } = createContext(
      (): Promise<User> => Promise.resolve(refreshedUser)
    );

    expect(service.activeUser()).toEqual(USER);
    await settle();

    expect(userBackendService.getUserById).toHaveBeenCalledOnceWith(USER.id);
  ...
  it('clears a persisted selection only when reconciliation confirms deletion', async () => {
    localStorage.setItem(ACTIVE_USER_STORAGE_KEY, JSON.stringify(USER));
    const { service, router } = createContext((): Promise<User> =>
      Promise.reject(new HttpErrorResponse({ status: 404 }))
    );
  ...
  it('retains a persisted selection when reconciliation temporarily fails', async () => {
    ...
    const { service, router } = createContext(
      (): Promise<User> => Promise.reject(new Error('offline'))
    );
  ...
  it('does not let a stale reconciliation override a later selection', async () => {
    ...
    localStorage.setItem(ACTIVE_USER_STORAGE_KEY, JSON.stringify(USER));
    const { service, router } = createContext(
      (): Promise<User> => deferredUser.promise
    );
